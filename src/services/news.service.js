...
- 문단 안 단어는 그 레벨 학습자가 이해할 수 있는 수준이어야 합니다.
- 반드시 해당 레벨 본문에 사용된 단어만 추출해야 합니다.
- 반드시 4개 레벨(A2, B1, B2, C1) 모두에 대해 콘텐츠와 단어를 생성해야 합니다.

[콘텐츠 생성 규칙]
 1. 레벨별 재작성: 하나의 기사를 기반으로 A2(기초), B1(중급), B2(중상급), C1(고급) 4개 레벨로 재작성해서 가져오세요.
  - A2: 매우 쉬운 어휘, 짧은 단문 위주 (약 1200자)
  - B1: 일상적 어휘, 명확한 문장 구조, 구동사 포함 (약 1500자)
  - B2: 전문 어휘 시작, 다양한 문장 구조 및 관용구 (약 1800자)
  - C1: 원문에 가까운 고급 어휘, 저널리즘 스타일, 세련된 표현 (약 2000자)

 2. 내용 구성: 서론/결론 등의 미사여구 없이 핵심 정보(경제·정치적 사실)만 포함하세요. 레벨이 높아질수록 정보의 깊이와 문맥의 복잡성을 더하세요.
 3. 언어 규칙: 모든 content와 title은 영어로 작성하며, meaning은 한국어로 작성합니다.
 4. 구조 규칙: * title: 해당 레벨 어휘로 작성된 영문 제목.
...
   - 의미(meaning) 란에 단순 사전적 정의가 아니라, 해당 레벨 학습자가 이해할 수 있는 **'쉬운 풀이'**를 한국어로 적으세요. (예: GDP -> 한 나라가 번 돈의 합계)
   ...
   - 개수는 해당 레벨 학습자가 문단을 읽을수 있을많큼 제공하세요.
4. 필드상세 :
  - meaning : 한국어 뜻.
  - example: 본문에 사용된 문장을 기반으로 하되, 학습자가 이해하기 쉽게 재구성한 영어 예문 1개.
  - example_meaning: 해당 예문의 한국어 해석.
